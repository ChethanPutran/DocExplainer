...
class PrerequisiteAnalyzer:
    ...
    def analyze_prerequisites(self, target_concept_name: str) -> Dict[str, Any]:
        ...
        prerequisites = concept_graph.get_prerequisites(
            target_concept_name, 
            user_knowledge.get_confidence(target_concept_name)
        )
        ...
        if target_concept:
            dependents = [
                c.name for c in concept_graph.get_dependents(target_concept.name)
            ]
    ...
    def detect_section_prerequisites(self) -> Dict[int, Set[int]]:
        ...
        assert self.graph_state_manager.document is not None, "Document must be set in GraphStateManager"
        ...
        for section in self.graph_state_manager.document.get_sections():
            for concept in section.concepts:
                ...
                concept_graph = self.knowledge_store.graph
                for dep in concept_graph.get_dependents(concept.name):
