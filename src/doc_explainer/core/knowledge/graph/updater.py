...
class GraphUpdater:
    ...
    def apply_delta(self, delta: GraphDelta, target_graph: ConceptGraph):
        ...
        for name, node in delta.new_concepts.items():
            if not self.knowledge_store.get_concept_node_by_name(name):
                self.knowledge_store.save_concept_node(node)
                target_graph.add_concept_node(node)

        # Add new relationships
        for edge in delta.new_edges:
            u = self.knowledge_store.get_concept_node_by_name(edge.concept1.primary_concept.name)
            v = self.knowledge_store.get_concept_node_by_name(edge.concept2.primary_concept.name)
            ...
            self.knowledge_store.save_concept_node_relationship(edge)
