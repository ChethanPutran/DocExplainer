...
class BaseResourceRecommender(ResourceRecommenderInterface, ABC):
    ...
    def recommend_khan_academy(self, concept: str, level: ExplanationLevel) -> Optional[Resource]:
        ...
        strategy = self.get_strategy_for_type(ResourceType.COURSE)
        if strategy:
            return strategy.recommend(concept, level)
        return self._fallback_recommendation(concept, level, ResourceType.COURSE)
    ...
    def _fallback_recommendation(self, concept: str, level: ExplanationLevel, 
        ...
        url = Resource.create_search_link(concept, level.name, platform)
