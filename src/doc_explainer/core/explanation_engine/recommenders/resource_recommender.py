...
from ...common.enums import ResourceType, ExplanationLevel
...
class ResourceRecommender(BaseResourceRecommender):
    ...
    def recommend_videos(self, concept: str, level: ExplanationLevel) -> Resource:
        ...
        query = self._build_search_query(concept, level.name)
        ...
        return Resource(
            ...
            difficulty=self._parse_level(level.name),
            source="YouTube"
        )
    ...
    def recommend_exercises(self, concept: str, level: ExplanationLevel) -> Resource:
        ...
        query = self._build_search_query(concept, level.name, include_terms=["practice", "exercises", "quiz"])
    ...
    def recommend_khan_academy(self, concept: str, level: ExplanationLevel) -> Optional[Resource]:
        ...
        query = self._build_search_query(concept, level.name, include_terms=["khan academy"])
    ...
    def recommend_by_type(self, concept: str, level: ExplanationLevel, 
        ...
        return Resource(
            ...
            url=Resource.create_search_link(concept, level.name),
            type=resource_type,
