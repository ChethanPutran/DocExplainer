...
import re
import json
from core.knowlege_modelling.graph.base import Concept, ConceptRelationship
...
class ConceptExtractor:
    def __init__(self, text_model, concepts_per_para: int = 10) -> None:
        self.llm = LLMWrapper(concept_refinement_template, json_output=True)
...
class RelationshipExtractor:
    def __init__(self) -> None:
        self.llm = LLMWrapper(relation_extractor_prompt, json_output=True)
    ...
    def _get_or_create_relationship(self, name: str) -> str:
        ...
        Also maps various relation phrases to a standardized set of relationship types.

        Parameters:
        - name: The raw relationship name or phrase.
        Returns:
        - A standardized relationship type string.
        """
    ...
    def _extract_llm(self, concepts: List[Concept], text: str, context: str) -> List[ConceptRelationship]:
        concept_names = [c.name for c in concepts]
        response_ = [{'source': 'self - attention mechanism', 'target': 'transformer', 'relation': 'depends_on', 'strength': 1.0, 'attributes': {'rationale': 'Transformers rely on self-attention mechanisms to model long-range dependencies.', 'context_type': 'theoretical'}}, {'source': 'sequence', 'target': 'transformer', 'relation': 'depends_on', 'strength': 0.85, 'attributes': {'rationale': 'Transformers model dependencies within sequences.', 'context_type': 'theoretical'}}, {'source': 'transformer', 'target': 'code generation', 'relation': 'enables', 'strength': 0.9, 'attributes': {'rationale': 'Transformers dominate text and code generation tasks.', 'context_type': 'practical'}}, {'source': 'transformer', 'target': 'gpt-5', 'relation': 'is_a', 'strength': 1.0, 'attributes': {'rationale': 'GPT-5 is listed as a representative transformer-based model.', 'context_type': 'practical'}},
                     {'source': 'transformer', 'target': 'gemini-3', 'relation': 'is_a', 'strength': 1.0, 'attributes': {'rationale': 'Gemini-3 is listed as a representative transformer-based model.', 'context_type': 'practical'}}, {'source': 'transformer', 'target': 'claude-4.5', 'relation': 'is_a', 'strength': 1.0, 'attributes': {'rationale': 'Claude-4.5 is listed as a representative transformer-based model.', 'context_type': 'practical'}}, {'source': 'random noise', 'target': 'diffusion', 'relation': 'depends_on', 'strength': 0.95, 'attributes': {'rationale': 'Diffusion models function by gradually denoising random noise into structured data.', 'context_type': 'theoretical'}}, {'source': 'diffusion', 'target': 'video generation', 'relation': 'enables', 'strength': 0.9, 'attributes': {'rationale': 'Diffusion models achieve state-of-the-art performance in video generation.', 'context_type': 'practical'}}]
        try:
            ...
            for item in response:
                ...
                if c1 and c2:
                    ...
                    relations.append(ConceptRelationship(
                                            ...
                                            attributes=item["attributes"].get("context_type", "")
                                        )
                                    )
                else:
                    print(
                        f"LLM returned relation with unknown concepts: {item['source']} or {item['target']}")
            return relations
        except Exception as e:
    ...
    def _extract_statistical_relationships(
        ...
        for _, rel_list in final_output:
    ...
    def extract(self, concepts: List[Concept], text: str, context: str) -> List[ConceptRelationship]:
        # Extract statistical relationships based on co-occurrence and proximity
        stat_relations = self._extract_statistical_relationships(
            concepts, text)
        
        # Extract relationships using LLM for deeper semantic understanding
        llm_relations = self._extract_llm(concepts, text, context)
...
if __name__ == "__main__":
    ...
    relationships = relationship_extractor.extract(
        concepts, text=text, context=text)
    for rel in relationships:
        print(
            f"Relationship: {rel.concept1.name} -[{rel.relation}]-> {rel.concept2.name}")
