...
class WindowManager:
    ...
    def on_explain(self, doc_id: str, text: str):
        response = self.pipeline.explain(doc_id=doc_id, selected_text=text)
        self.main_window.sidebar.update_explanation(response)
        self.main_window.voice_output.set_text(response.explanation)

    def on_summarize(self, doc_id: str, text: str):
        response = self.pipeline.summarize(doc_id=doc_id, selected_text=text)
        self.main_window.sidebar.update_explanation(response.explanation)

    def on_ask(self, doc_id: str, text: str):
        response = self.pipeline.answer_question(doc_id=doc_id, selected_text=text)
        self.main_window.sidebar.update_explanation(response.explanation)
